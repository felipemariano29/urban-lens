"""Pure medallion transformations for crimes data."""
...
from urban_lens.contracts import CSV_COLUMN_ALIASES, SILVER_OUTPUT_COLUMNS, SUPPORTED_POLICE_UK_DATASET_KIND
...
def build_ml_datasets(area_month_category_frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    scoring_set = features[features["incident_count_next_period"].isna()].drop(columns=["incident_count_next_period"])
    return training_set, scoring_set.reset_index(drop=True)
